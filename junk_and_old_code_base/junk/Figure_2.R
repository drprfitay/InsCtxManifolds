...
figure_2_pairwise_topological_permutation_generic <- function(metadata_1, metadata_2)
{
  ...
  for (i1 in 1:len(metadata_1)) {
    ...
    for (i in 1:nreps) {
      ...
      ind2 <- which(!logical_ind)
        ...
        for (metric_name in names(metric_functions)) {
          ...
          if (len(perm_life_death_1_mat_1) == 0 || len(perm_life_death_1_mat_1) == 0) {
            next
          }
    ...
    for (metric_name in names(metric_functions)) {
      ...
      pvalue_matrices_betti_0[[metric_name]][i1, i2] <- 1 - pval_func_betti_0(phomology_similarity[[metric_name]]$betti_0)
      pvalue_matrices_betti_1[[metric_name]][i1, i2] <- 1 - pval_func_betti_1(phomology_similarity[[metric_name]]$betti_1)
    }
...
metadata_v1_por <- across_mice_decoding_build_metadata(control_paths, control=T)
    ...
    for (comp_conf_name in names(comparisions)) {
      ...
      for (comp in unique(unlist(comp_conf$comp_ind))){
        comps_df <- rbind(comps_df,
                          data.frame(similarity=vals[[(comp)]],
                                     x=rep(labels[[(comp)]], times=len(vals[[(comp)]]))))
        
        pvalues_df <- rbind(pvalues_df,
                            data.frame(pval=pvals[[(comp)]],
                                       x=rep(labels[[(comp)]], times=len(pvals[[(comp)]]))))
      }
  ...
  for (barc in c("betti0", "betti1")) {
    ...
    ph_pval_unc <- 
      ...
      for (size_name in names(sizes)) {
        ...
        pdf(sprintf("%s\\unclustered_similarity_heatmap_%s_metric_%s_barc_%s.pdf",
                    tmp_write_path,
                    comp_name,
                    metric,
                    barc),
        ...
        plot(ph_similarity_unc[[4]])
        dev.off()
        
        pdf(sprintf("%s\\unclustered_pval_heatmap_%s_metric_%s_barc_%s.pdf",
                    tmp_write_path,
                    comp_name,
                    metric,
                    barc),
