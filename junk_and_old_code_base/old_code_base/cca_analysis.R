...
# get_behavior_variables <- function(stim_master_mat, mat_size, window_size=30, seconds_after=2, control=F) {
...
#   
#   if (!control) {
#     false_alarms <- !is.nan(stim_master_mat[,11]) & stim_master_mat[,"TrialType"] %in% c(4)
#   cont_false_alarm_ind <- sapply(stim_master_mat[false_alarms, 11],
#                                  function(i) {get_binned_index(i:(i + seconds_after * window_size), window_size)})
#   }
#   
#   tt3_by_ind <- cor_var_base
#   tt3_by_iti <- cor_var_base
#   ind <- which(stim_master_mat[,"TrialType"] == 3)
#   res <- lapply(ind[-(len(ind))], function(i) {stim_master_mat[i,1]:stim_master_mat[i+1:1]})
#   tt3_inds <- lapply(res, function(ind) {unique(sapply(ind, function(i) {get_binned_index(i, window_size)}))})
#   for (idx in 1:len(tt3_inds)) {
#     
#     indices <- tt3_inds[[idx]]
#     tt3_by_ind[indices] <- idx
#     tt3_by_iti[indices[(len(indices) - 5):len(indices)]] <- idx
#     
#   }
#   
#   tt4_by_ind <- cor_var_base
#   tt4_by_iti <- cor_var_base
#   ind <- which(stim_master_mat[,"TrialType"] == 4)
#   res <- lapply(ind[-(len(ind))], function(i) {stim_master_mat[i,1]:stim_master_mat[i+1:1]})
#   tt4_inds <- lapply(res, function(ind) {unique(sapply(ind, function(i) {get_binned_index(i, window_size)}))})
#   for (idx in 1:len(tt4_inds)) {
#     indices <- tt4_inds[[idx]]
#     tt4_by_ind[indices] <- idx
#     tt4_by_iti[indices[(len(indices) - 5):len(indices)]] <- idx
#     
#     
#     
#   }
#   
#   tt5_by_ind <- cor_var_base
#   tt5_by_iti <- cor_var_base
#   ind <- which(stim_master_mat[,"TrialType"] == 5)
#   res <- lapply(ind[-(len(ind))], function(i) {stim_master_mat[i,1]:stim_master_mat[i+1:1]})
#   tt5_inds <- lapply(res, function(ind) {unique(sapply(ind, function(i) {get_binned_index(i, window_size)}))})
#   for (idx in 1:len(tt5_inds)) {
#     indices <- tt5_inds[[idx]]
#     tt5_by_ind[indices] <- idx
#     tt5_by_iti[indices[(len(indices) - 5):len(indices)]] <- idx
#     
#   }
#   
#   rewards <- cor_var_base
#   rewards[cont_rewards_ind] <- 1
#   false_alarms <- cor_var_base
#   
#   if (!control) {
#     false_alarms[cont_false_alarm_ind] <- 1
#   }
#   
...
analyse_modes <- function(window_size=30, mat_frames=57600) {
  ...
  lapply(c(1:len(paths_all)),
      ...
      for (compare_method_name in names(comparsion_methods_vec)) {
        ...
        filter_out <- grepl(c("TrialType 3"), compare_df $variable) |
                      grepl(c("TrialType 4"), compare_df $variable) |
                      grepl(c("TrialType 5"), compare_df $variable) |
                      grepl(c("TrialType 5"), compare_df $variable) | 
                      grepl("Smoothed cumulative 33", compare_df$variable) |
                      grepl(c("Reward"), compare_df $variable) |
                      grepl(c("reward"), compare_df $variable)

        compare_df_2 <- compare_df[filter_out,]
        
        
        compare_modes_graph <-  ggplot(compare_df_2, aes(x=variable, y=value)) + 
                            geom_boxplot(aes(group=interaction(categories, variable), fill=categories),
                                         width=0.5, size=1) + 
                            ...
                            scale_fill_manual(name="Task",
                                              ...
                                              values=adjustcolor(c("goldenrod2", "royalblue4", "darksalmon" ), alpha=0.8),
